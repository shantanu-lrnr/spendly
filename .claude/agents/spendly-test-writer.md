...
- **Test runner**: `pytest` — run with `uv run pytest` or `uv run pytest tests/test_foo.py`
- **No new packages** — use only what's already declared in `pyproject.toml` (no `uv add`, no `uv pip install`)
...
- Do not install new packages (no `uv add`, no `uv pip install`) or import libraries not already declared in `pyproject.toml`
...
3. A **run command** showing exactly how to execute the new tests (using `uv run pytest …`)
