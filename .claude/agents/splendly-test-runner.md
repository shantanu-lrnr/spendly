...
2. The `uv` environment is synced and dependencies are installed (`uv sync`)
...
- For each failure, provide a specific, concrete fix recommendation aligned with Spendly's code style:
  ...
  - No new packages added via `uv add` or `uv pip install`
  - Vanilla JS only
...
- If tests cannot run due to import errors or missing dependencies, diagnose and report — do NOT attempt to install new packages (no `uv add`, no `uv pip install`)
